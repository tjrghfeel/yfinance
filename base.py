...
class TickerBase():
    ...
    def history(self, period="1mo", interval="1d",
        ...
        data = self.session.get(
            ...
            # 기존 yfinance에 추가한 코드. 
            # 요청에 대한 timeout이 없어 너무 오래 대기하여 추가. 
            timeout=30, 
        )

        # 기존 yfinance에 추가한 코드. 
        # status code!=200인 경우에 대한 처리 없는 듯함. 좀 아래의 에러처리는 
        # 응답은 정상적으로 왔으나 그 내용이 야후에서 보낸 에러인 경우를 처리하는것. 
        # 하여, 응답코드가 200이 아닌 경우를 명시적으로 처리하는게 나을듯하여 추가. 
        # 처리 방법은, yfinance상에서 데이터 요청에 대한 응답이 제대로 안와도 shared._DFS에 빈 df라도 넣는 방식으로
        # 처리하고 있는듯하여, 예외를 던지기보단 동일하게 처리. 
        if data.status_code != 200: 
            # raise Exception(f"request 에러. {data.status_code}, {data.text}")
            err_msg = f"request 에러. {data.status_code}, {data.text}"
